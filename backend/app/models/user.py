...
class RolePermission(Base):
    ...
    id = Column(UUIDColumn, primary_key=True, default=uuid.uuid4)
    role_id = Column(UUIDColumn, ForeignKey("roles.id", ondelete="CASCADE"))
    permission_id = Column(UUIDColumn, ForeignKey("permissions.id", ondelete="CASCADE"))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class User(Base):
    ...
    role_id = Column(UUIDColumn, ForeignKey("roles.id"))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class UserProfile(Base):
    ...
    id = Column(UUIDColumn, primary_key=True, default=uuid.uuid4)
    user_id = Column(UUIDColumn, ForeignKey("users.id", ondelete="CASCADE"))
    bio = Column(Text)
